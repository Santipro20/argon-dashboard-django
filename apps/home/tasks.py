...
def get_data_id():
    ...
    if cached_result is not None:
        ...
        df = df[
            (df["created_by_id"] == 1)
            | (df["merchant_id_course"].isin(merchant_trd_ids))
            | (df["merchant_id_course"].isin(merchant_id_ids))
        ]
...
def get_data_cydi():
    ...
    if cached_result is not None:
        ...
        df_geo_task = df_geo_task[
            ["odometer", "timestamp", "id", "coordinates", "history"]
        ]
        ...
        speed_mean_cycling = (
            1 + 2.8 / 17.8
        )  # Porcentage of speed more fast than normal cycling

        df_task["durations_cycling_copy"] = df_normalized_1["durations_cycling"].apply(
            lambda x: copy.deepcopy(x) if isinstance(x, list) else []
        )
        df_task["speeds_cycling_copy"] = df_normalized_1["speeds_cycling"].apply(
            lambda x: copy.deepcopy(x) if isinstance(x, list) else []
        )
        df_task["distances_cycling_copy"] = df_normalized_1["distances_cycling"].apply(
            lambda x: copy.deepcopy(x) if isinstance(x, list) else []
        )

        df_task["durations_cycling_copy"] = df_task["durations_cycling_copy"].apply(
            lambda x: [value * mean_time for value in x]
            if isinstance(x, list)
            and all(isinstance(value, (int, float)) for value in x)
            else x
        )
        df_task["speeds_cycling_copy"] = df_task["speeds_cycling_copy"].apply(
            lambda x: [value * speed_mean_cycling for value in x]
            if isinstance(x, list)
            and all(isinstance(value, (int, float)) for value in x)
            else x
        )
        df_task["distances_cycling_copy"] = df_task.apply(
            lambda row: [
                a * b
                if isinstance(a, (int, float)) and isinstance(b, (int, float))
                else None
                for a, b in zip(
                    row["durations_cycling_copy"], row["speeds_cycling_copy"]
                )
            ]
            if isinstance(row["durations_cycling_copy"], list)
            and isinstance(row["speeds_cycling_copy"], list)
            else row["durations_cycling_copy"],
            axis=1,
        )

        df_grouped = df_geo_task.groupby("id").apply(
            lambda group: group.sort_values(by="timestamp")
        )
        ...
        df_grouped["three.datetime_history"] = pd.to_datetime(
            df_grouped["three.datetime_history"], format="%Y-%m-%dT%H:%M"
        )
        df_grouped["two.datetime_history"] = pd.to_datetime(
            df_grouped["two.datetime_history"], format="%Y-%m-%dT%H:%M"
        )
        df_grouped["timestamp"] = pd.to_datetime(
            df_grouped["timestamp"], format="%Y-%m-%dT%H:%M"
        )
        ...
        for i in range(len(id_gr)):
            ...
            for r in range(
                len(df_ex) - 1
            ):  # Resta 1 para evitar un índice fuera de rango
                a = df_ex.iloc[r + 1]["odometer"] - df_ex.iloc[r]["odometer"]
                if pd.isna(df_ex.iloc[r]["three.datetime_history"]):
                    t = (
                        df_ex.iloc[r + 1]["timestamp"] - df_ex.iloc[r]["timestamp"]
                    ).total_seconds()
                else:
                    t = (
                        df_ex.iloc[r]["three.datetime_history"]
                        - df_ex.iloc[r]["two.datetime_history"]
                    ).total_seconds()
                c = a / t
                vector.append(a)
                vector_1.append(t)
        ...
        df_geo_task = pd.DataFrame(
            {
                "id": list_4,
                "distance_deki": list_1,
                "durations_deki": list_2,
                "speeds_deki": list_3,
            }
        )

        for i in range(len(df_geo_task)):
            df_geo_task.loc[i, "Total_distance_deki"] = sum(
                df_geo_task.loc[i, "distance_deki"]
            )
            df_geo_task.loc[i, "Total_durations_deki"] = sum(
                df_geo_task.loc[i, "durations_deki"]
            )
        ...
        columns_prona = [
            "speeds_cycling_copy",
            "distances_cycling_copy",
            "durations_cycling_copy",
            "totalDistance_cycling",
            "totalDuration_cycling",
            "speeds_driving",
            "distances_driving",
            "durations_driving",
            "totalDistance_driving",
            "totalDuration_driving",
            "deliveryPoint_altitudes",
            "pickUpPoint_altitudes",
        ]
        df_geo_task = df_geo_task.dropna(subset=columns_prona, how="any")
        df_geo_task = df_geo_task[
            (df_geo_task["totalDuration_driving"] != 0)
            | (df_geo_task["totalDuration_cycling"] != 0)
        ]
        df_geo_task = df_geo_task[
            (df_geo_task["totalDistance_driving"] != 0)
            | (df_geo_task["totalDistance_driving"] != 0)
        ]
    ...
    if cached_result is not None:
        ...
        if newuser is not None:
            if (
                newuser == "group_id=1"
                or newuser == 1
                or newuser == "index2.html/group_id=1"
            ):
                merchant_trd = df_merchants[df_merchants["group_0_id"] == 1]
                merchant_trd = merchant_trd[["id"]]
                merchant_trd_ids = merchant_trd["id"].tolist()
...
def CO2(stored_selected_date, selected_city, newuser=None):
    ...
    if cached_result is not None:
        ...
        if stored_selected_date is not None:
            ...
            df_geo_task = df_geo_task[
                (df_geo_task["end_date"] >= st) & (df_geo_task["end_date"] <= en)
            ]
        ...
        if df_geo_task.empty:
            ...
            mask = (df_geo_task["pickUpPoint_altitudes"] == 0) | (
                df_geo_task["deliveryPoint_altitudes"] == 0
            )
            df_geo_task.loc[mask, "inclination"] = (
                df_geo_task["pickUpPoint_altitudes"]
                + df_geo_task["deliveryPoint_altitudes"]
            )
            ...
            df_geo_task.loc[mask, "inclination"] = (
                ...
                / df_geo_task["totalDistance_driving"]
            ) * 100
            ...
            df_geo_task["speeds_cycling_copy"] = df_geo_task[
                "speeds_cycling_copy"
            ].apply(lambda x: [x] if isinstance(x, int) else x)
            vector_diffs_1 = (
                df_geo_task["speeds_cycling_copy"].apply(calculate_diffs).tolist()
            )

            df_geo_task["speeds_deki"] = df_geo_task["speeds_deki"].apply(
                clean_speeds_cycling
            )
            df_geo_task["speeds_deki"] = df_geo_task["speeds_deki"].apply(
                lambda x: [x] if isinstance(x, int) else x
            )
            vector_diffs_1_1 = (
                df_geo_task["speeds_deki"].apply(calculate_diffs).tolist()
            )

            # Apply the function to the 'speeds_driving' column
            df_geo_task["speeds_driving"] = df_geo_task["speeds_driving"].apply(
                lambda x: [x] if isinstance(x, int) else x
            )
            vector_diffs_2 = (
                df_geo_task["speeds_driving"].apply(calculate_diffs).tolist()
            )
            df_geo_task["acceleration_driving"] = vector_diffs_2

            condition = (
                (
                    df_geo_task["Total_distance_deki"]
                    > df_geo_task["totalDistance_cycling"]
                )
                | (df_geo_task["Total_distance_deki"] == 0)
                | (
                    df_geo_task["Total_durations_deki"]
                    > df_geo_task["totalDuration_cycling"]
                )
            ...
            for i, row in df_geo_task.iterrows():
                if condition.iloc[i]:
                    df_geo_task.at[i, "speeds_cycling_copy"] = df_geo_task[
                        "speeds_cycling_copy"
                    ][i]
                else:
                    df_geo_task.at[i, "speeds_cycling_copy"] = df_geo_task[
            ...
            for i, row in df_geo_task.iterrows():
                if condition.iloc[i]:
                    df_geo_task.at[i, "distances_cycling_copy"] = df_geo_task[
                        "distances_cycling_copy"
                    ][i]
                else:
                    df_geo_task.at[i, "distances_cycling_copy"] = df_geo_task[
            ...
            for i, row in df_geo_task.iterrows():
                if condition.iloc[i]:
                    df_geo_task.at[i, "durations_cycling_copy"] = df_geo_task[
                        "durations_cycling_copy"
                    ][i]
                else:
                    df_geo_task.at[i, "durations_cycling_copy"] = df_geo_task[
                ...
                if isinstance(list_cell, list):
                    for i in range(len(list_cell)):
                        ...
                        if speed_value is not None:
                            b2_value.append(
                                (0.5 * Density_wind * area_front_vul * speed_value * cd)
                                / 1000
                            )
                        else:
                            b2_value.append(
                                None
                            )  # You can choose to keep None if needed
                else:
                    if list_cell is not None:
                        b2_value.append(
                            (0.5 * Density_wind * area_front_vul * list_cell * cd)
                            / 1000
                        )
                    else:
            ...
            for n in range(len(df_geo_task)):
                ...
                if isinstance(list_cell, list) and isinstance(list_cell2, list):
                    for i in range(len(list_cell)):
                        b2_list = b2[n]
                        if isinstance(list_cell2[i], (int, float)) and isinstance(
                            list_cell[i], (int, float)
                        ):
                            rt_value = (
                                b1
                                + b2_list[i] * (list_cell2[i] ** 2)
                                + ((sum_weight_vul * list_cell[i]) / 1000)
                                + (
                                    (
                                        gravity
                                        * sum_weight_vul
                                        * df_geo_task["inclination"][n]
                                    )
                                    / 100000
                                )
                            )
                    ...
                    b2_list = b2[n]
                    if isinstance(list_cell2, (int, float)) and isinstance(
                        list_cell, (int, float)
                    ):
                        rt_value = (
                            b1
                            + b2_list * (list_cell2**2)
                            + ((sum_weight_vul * list_cell) / 1000)
                            + (
                                (
                                    gravity
                                    * sum_weight_vul
            ...
            for n in range(len(rt)):
                if not rt[n]:  # Check if the list at index n is empty
                    rt[n] = [
                        0
                    ]  # Replace the empty list with a list containing a single zero
                    ...
                    if list_cell <= 0:
                        ...
                        ft_value.append(
                            (
                                (
                                    alpha_vul
                                    + beta_1_vul_diseal * list_cell * list_cell3
                                    + (
                                        beta_2_vul_diseal
                                        * sum_weight_vul
                                        * (list_cell2**2)
            ...
            df_geo_task["totalDistance_cycling"] = df_geo_task[
                "distances_cycling_copy"
            ].apply(sum)
            ...
            df_geo_task.loc[mask, "inclination_cycling"] = (
                df_geo_task["pickUpPoint_altitudes"]
                + df_geo_task["deliveryPoint_altitudes"]
            )
                ...
                if isinstance(list_cell, list) and isinstance(list_cell2, list):
                    for i in range(len(list_cell)):
                        ...
                        if isinstance(list_cell2[i], (int, float)) and isinstance(
                            ...
                            BSP_value = list_cell2[i] * (
                                1.01 * list_cell[i] + 9.81 * np.sin(inclination) + 0.078
                            ) + new_cof * (list_cell2[i] ** 3)
                            BSP_value = (
                                BSP_value * sum_weight_velo * list_cell3[i] / 1000
                            )
                    ...
                    if isinstance(list_cell2, (int, float)) and isinstance(
                        ...
                        BSP_value = list_cell2 * (
                            1.01 * list_cell + 9.81 * np.sin(inclination) + 0.078
                        ) + new_cof * (list_cell2**3)
                ...
                if isinstance(list_cell, list) and isinstance(list_cell2, list):
                    for i in range(len(list_cell)):
                        ...
                        if isinstance(list_cell2[i], (int, float)) and isinstance(
                            ...
                            VSP_value = list_cell2[i] * (
                                1.1 * list_cell[i] + 9.81 * np.sin(inclination) + 0.132
                            ) + new_cof2 * (list_cell2[i] ** 3)
                            VSP_value = (
                                VSP_value * sum_weight_vul * list_cell3[i] / 1000
                            )
                    ...
                    if isinstance(list_cell2, (int, float)) and isinstance(
                        ...
                        VSP_value = list_cell2 * (
                            1.1 * list_cell + 9.81 * np.sin(inclination) + 0.132
                        ) + new_cof2 * (list_cell2**3)
            ...
            if CO2_driving_total != 0:
                emi = round(
                    ((CO2_driving_total - CO2_cycling_total) / CO2_driving_total) * 100,
                    2,
                )
            else:
                emi = 0
            ...
            result = (
                emi,
                emi_total,
                round(CO2_driving_total, 2),
                round(CO2_cycling_total, 2),
                round(CO2_deki_vul_total, 2),
            )
    ...
    if cached_result is not None:
        ...
        if df_geo_task.empty:
            ...
            if distance_driving > 0:
                km_saved = round(
                    ((distance_driving - distance_cycling) / distance_driving) * 100, 2
                )
            else:
                km_saved = 0
        ...
        result = (
            km_saved,
            km_saved_total,
            round(distance_cycling),
            round(distance_driving),
            round(mean_cycling),
        )
...
def time_eco(stored_selected_date, selected_city, newuser=None):
    ...
    if cached_result is not None:
        ...
        if df_geo_task.empty:
            ...
            df_geo_task["durations_cycling_copy"] = df_geo_task[
                "durations_cycling_copy"
            ].apply(lambda x: [x] if isinstance(x, int) else x)
            df_geo_task["durations_driving"] = df_geo_task["durations_driving"].apply(
                lambda x: [x] if isinstance(x, int) else x
            )
            time_cycling = sum(
                val
                for sublist in df_geo_task["durations_cycling_copy"]
                if isinstance(sublist, list)
                for val in sublist
            )
            time_driving = sum(
                val
                for sublist in df_geo_task["durations_driving"]
                if isinstance(sublist, list)
                for val in sublist
            )
            ...
            if time_driving > 0:
                time_saved = round(
                    ((time_driving - time_cycling) / time_driving) * 100, 2
                )
            else:
                time_saved = 0
            ...
            df_geo_task["sum_of_durations"] = df_geo_task[
                "durations_cycling_copy"
            ].apply(sum_list)
            ...
            result = (
                time_saved,
                time_saved_total,
                rounded_time_cycling,
                round(time_driving / 3600),
                media_time_cycling,
            )
...
def equivalence(stored_selected_date, selected_city, newuser=None):
    ...
    if cached_result is not None:
        ...
        if newuser is not None:
            emi, emi_total, driving, cycling, deki_vul = CO2(
                stored_selected_date, selected_city, newuser
            )
        else:
            emi, emi_total, driving, cycling, deki_vul = CO2(
                stored_selected_date, selected_city
            )
    ...
    if cached_result is not None:
        ...
        if stored_selected_date is not None:
            ...
            df_packages = df_packages[
                (df_packages["created_at"] >= st) & (df_packages["created_at"] <= en)
            ]
            df = df[(df["end_date"] >= st) & (df["end_date"] <= en)]
        else:
            df_geo_task = df_geo_task
        ...
        if newuser is not None:
            ...
            km_saved, km_saved_total, driving, l, m = eco_km(
                stored_selected_date, selected_city, newuser
            )
            time_saved, time_saved_total, d, c, f = time_eco(
                stored_selected_date, selected_city, newuser
            )
            SC_s, SC_vul, SC_deki = space_eco(
                stored_selected_date, selected_city, newuser
            )
            if newuser == "group_id=1" or newuser == 1:
                merchant_trd = df_merchants[df_merchants["group_0_id"] == 1]
        ...
        if df_geo_task.empty or df_packages.empty:
            ...
            df_geo_task["new_weight"] = df_geo_task["notes"].apply(
                lambda x: extract_weight(x) if pd.notna(x) else None
            )
            df_geo_task["new_weight"] = (
                df_geo_task["new_weight"]
            ...
            df_geo_task["cartons"] = df_geo_task["notes"].apply(
                lambda x: extract_cartons(x) if pd.notna(x) else None
            )
            df_geo_task["cartons"] = (
                df_geo_task["cartons"].str.replace(r"\D", "", regex=True).astype(float)
            )
            ...
            df_geo_task["bidons"] = df_geo_task["notes"].apply(
                lambda x: extract_cartons(x) if pd.notna(x) else None
            )
            df_geo_task["bidons"] = (
                df_geo_task["bidons"].str.replace(r"\D", "", regex=True).astype(float)
            )
            ...
            df_geo_task["sceaux"] = df_geo_task["notes"].apply(
                lambda x: extract_cartons(x) if pd.notna(x) else None
            )
            df_geo_task["sceaux"] = (
                df_geo_task["sceaux"].str.replace(r"\D", "", regex=True).astype(float)
            )
            ...
            df_geo_task["bouteilles"] = df_geo_task["notes"].apply(
                lambda x: extract_cartons(x) if pd.notna(x) else None
            )
            df_geo_task["bouteilles"] = (
                df_geo_task["bouteilles"]
                .str.replace(r"\D", "", regex=True)
                .astype(float)
            )
            ...
            df_geo_task["sacs"] = df_geo_task["notes"].apply(
                lambda x: extract_cartons(x) if pd.notna(x) else None
            )
            df_geo_task["sacs"] = (
                df_geo_task["sacs"].str.replace(r"\D", "", regex=True).astype(float)
            )
            ...
            df_geo_task["bonbonnes"] = df_geo_task["notes"].apply(
                lambda x: extract_cartons(x) if pd.notna(x) else None
            )
            df_geo_task["bonbonnes"] = (
                df_geo_task["bonbonnes"]
                .str.replace(r"\D", "", regex=True)
                .astype(float)
            )
            ...
            df_counts_colis = pd.DataFrame(
                {"delivery_id": count_colis.index, "colis_number": count_colis.values}
            )
            ...
            df_colis_detec["colis_number"] = df_geo_task[col_num].sum(
                axis=1, skipna=True
            )
            df_colis_detec["colis_number"] = df_colis_detec["colis_number"].replace(
                0.0, np.nan
            )
            ...
            df_colis["colis_number_updated"] = df_colis.apply(
                update_colis_number, axis=1
            )
            ...
            summarized_df = (
                df_packages.groupby("delivery_id")["weight"].sum().reset_index()
            )
            ...
            def weight(row):
                if pd.isna(row["new_weight"]):
                    result = (
                        df_colis_detec.at[row.name, "bidons"] * 3
            ...
            def final_weight(row):
                if pd.isna(row["final_weight"]):
                    if (
                        df_colis_detec.at[row.name, "id"]
                        in summarized_df["delivery_id"].values
                    ):
                        matching_rows = summarized_df.loc[
                            summarized_df["delivery_id"]
                            == df_colis_detec.at[row.name, "id"],
                            "weight",
                        ]
            ...
            df_colis_detec["final_weight_2"] = df_colis_detec.apply(
                final_weight, axis=1
            )
    ...
    if cached_result is not None:
        ...
        def convert_to_json(det):
            ...
            for i in range(len(det)):
                data.append(
                    {"x": det.iloc[i]["date"], "y": float(det.iloc[i]["count"])}
                )
        ...
        if df.empty:
            ...
            ds_delivery["day_of_week_fr"] = ds_delivery["day_of_week"].map(
                name_week_france
            )
            ...
            if season == "Journalier" or season is None:
                ...
                df_colis["colis_number_updated"] = df_colis[
                    "colis_number_updated"
                ].fillna(0)
                day_colis = (
                    df_colis.groupby(df_colis["end_date"].dt.date)[
                        "colis_number_updated"
                    ]
                    .sum()
                    .reset_index()
                ...
                df_colis_detec["final_weight_2"] = df_colis_detec[
                    "final_weight_2"
                ].fillna(0)
                day_weight = (
                    df_colis_detec.groupby(df_colis_detec["end_date"].dt.date)[
                        "final_weight_2"
                    ]
                    .sum()
                    .reset_index()
                ...
                week_delivery = (
                    df.groupby(df["end_date"].dt.to_period("W")).size().reset_index()
                )
                ...
                week_colis = (
                    df_colis.groupby(df_colis["end_date"].dt.to_period("W-Mon"))[
                        "colis_number_updated"
                    ]
                    .sum()
                    .reset_index()
                ...
                week_weight = (
                    df_colis_detec.groupby(
                        df_colis_detec["end_date"].dt.to_period("W-Mon")
                    )["final_weight_2"]
                    .sum()
                    .reset_index()
                )
                ...
                month_delivery = (
                    df.groupby(df["end_date"].dt.to_period("M")).size().reset_index()
                )
                ...
                month_colis = (
                    df_colis.groupby(df_colis["end_date"].dt.to_period("M"))[
                        "colis_number_updated"
                    ]
                    .sum()
                    .reset_index()
                ...
                month_weight = (
                    df_colis_detec.groupby(
                        df_colis_detec["end_date"].dt.to_period("M")
                    )["final_weight_2"]
                    .sum()
                    .reset_index()
                )
...
def graph_jk(stored_selected_date, selected_city, newuser=None):
    ...
    if cached_result is not None:
        ...
        if df.empty:
            ...
            df["end_date"] = pd.to_datetime(df["end_date"])
            df["diff_days"] = (
                df["start_date"] - df["created_at_task"]
            ...
            def asig(row):
                if (
                    (row["created_hour"] < 9 or row["created_hour"] > 18)
                    and row["diff_days"] <= 24
                    and (row["end_hour"] >= 8 or row["end_hour"] <= 18)
                ):
                    return "a"
                elif (row["created_hour"] >= 9 and row["created_hour"] <= 18) and row[
                    "diff_days"
                ] <= 33:
                    return "b"
            ...
            df_colis["colis_number_updated"] = df_colis["colis_number_updated"].fillna(
                0
            )
...
def livra_state(stored_selected_date, selected_city, newuser=None):
    ...
    if cached_result is not None:
        ...
        merchant_id_ids = merchant_id["id"].tolist()
        df_task = df_task[
            (df_task["created_by_id"] == 1)
            | (df_task["merchant_id_course"].isin(merchant_trd_ids))
            | (df_task["merchant_id_course"].isin(merchant_id_ids))
        ]
